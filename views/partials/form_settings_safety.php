<?php
/**
 * views/partials/form_settings_safety.php (V22.0)
...
<div class="box" style="grid-column: span 4; display: flex; flex-direction: column;">
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:10px;">
        ...
        <div>
            ...
            <input type="number" name="m_sf_heat" step="0.05" value="<?= $inputs['m_sf_heat'] ?? '1.20' ?>">
        </div>
    </div>
    
    <!-- Latent & Internal Temp Row -->
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; margin-bottom:10px;">
        <div>
            <label>SF Latent (Υγρασία)</label>
            <input type="number" name="m_latent" step="0.01" value="<?= $inputs['m_latent'] ?? '1.18' ?>">
        </div>
        <div>
            <label>Custom Tin °C</label>
            <input type="number" name="custom_tin" placeholder="Auto" value="<?= $inputs['custom_tin'] ?? '' ?>">
        </div>
    </div>

    <!-- Outdoor Temp Override -->
    <label>Custom Tout °C (Εξωτερική)</label>
    <input type="number" name="custom_tout" placeholder="Auto" value="<?= $inputs['custom_tout'] ?? '' ?>" style="margin-bottom: 20px;">
